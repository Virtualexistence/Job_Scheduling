...
import os
class job_shop_env():
    path = 'drive/MyDrive/Job_Schedule/data'
    expert_job = pd.read_csv(os.path.join(path,'process_time_matrix.csv'),header=None).drop([0]).values
    job = pd.read_csv(os.path.join(path + 'work_order.csv',header=None)).values
